...
| 함수명 | 설명 |
| :--- | :--- |
| `set_init_ER(self)` | ER 모델로 그래프를 생성하기 위해, 평균 차수와 노드수를 이용해 엣지 연결 확률을 계산하고 소수 2번째 자리까지 반환하도록 해 self.p에 저장한다. <br> 엣지 연결 확률 계산은 노드 수 $N$과 평균 차수 $\langle k \rangle$을 사용하여 근사할 수 있다. $$p \approx \frac{\langle k \rangle}{N-1}$$ <br> 예외 : (1) $N$가 0 일 때와 (2) 엣지 수가 0 일 때는 연결 확률을 계산할 수 없도록 한다.|
| `create_ERnp_graph(self)` | 원본 그래프의 노드와 set_init_ER에서 만든 연결 확률 $p$ 이용해 ER 그래프를 만들도록 한다. <br> 예외 : 연결 확률 $p$가 0과 1 사이의 값이 아닌 경우 해당 그래프를 만들 수 없도록 한다.| 
...
| 함수명 | 설명 |
| :--- | :--- |
| `create_config_graph(self)` | Configuration graph을 생성하는 데 사용하는 함수이다. <br> 동일한 이웃수 배열을 가지도록 한다. <br> 예외 : (1) 차수 합이 짝수가 아닐 때 그래프를 만들 수 없도록 한다.<br> (2) 모든 차수가 0 이상이 되지 않는다면 그래프를 만들 수 없도록 한다.|
4. 
