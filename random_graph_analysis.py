...
class RandomGraphAnalysis():
  ...
  def create_chunglu_graph(self):
    ...
    담당자: 김시현
    예외처리: (1) 차수 0 이상 확인 (2) 차수 총합 0 아님 확인 (3) 연결 확률 1 이하 확인
    '''

    n = len(self.degree_seq)
    ...
    # 1-3. 연결 확률 초과 체크
    if max_prob > 1.0:
      raise ValueError(f"계산된 최대 연결 확률 ({max_prob:.4f})이 1.0을 초과합니다. 모델 가정을 위배합니다.")
    ...
    for i, j in itertools.combinations(G.nodes(), 2):
      p_ij = (self.degree_dict[i] * self.degree_dict[j]) / total_degree
